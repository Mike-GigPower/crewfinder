---
title: 3.5.6
summary: THE GOAT — v3.5.6 Venue geo for Crew Finder distance search now comes from real SmartStaff venue data instead of a hand-maintained postcode…
updated: 2026-06-11
---
...
Exact venue-name match (the call's venue from get-calls-bulk is the
...
centroid.
Short code / keyword ("Forum", "JCA", a detect_venue code) → routed
through INDUCTION_VENUE_MAP to the matching cached venue, then its real geo.
Legacy VENUE_POSTCODES — last resort only, for the cold-start window
