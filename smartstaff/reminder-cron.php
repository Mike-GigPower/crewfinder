<?php
	...
	if ( $si->connect( $settings->smsuser, $settings->smspass, true, false ) )  {
		...
		$futureDate = strtotime('now + 24 hours');
		...
		$reminders = $db->select(
			...
			'call_crew_map.status=5 AND reminder_sent=0 AND calls.start_date > '. $currDate .' AND calls.start_date <= '. $futureDate
		);
		
		echo '[DEBUG] Reminders: ' . count($reminders)  . ' <br />';
